...
func TestNewVideoInfoFromFilepath_Success(t *testing.T) {
	...
	// When calling NewVideoInfoFromFilepath
	vid, err := NewVideoInfoFromFilepath("dummy.mp4")

	// Then no error and fields are set correctly
...
func TestNewVideoInfoFromFilepath_NoVideoStream(t *testing.T) {
	// Given ffprobe JSON with only an audio stream
	...
	// Then error about missing video stream
	require.Error(t, err)
...
func TestNewVideoInfoFromFilepath_InvalidDuration(t *testing.T) {
	// Given ffprobe JSON with a non-numeric duration
	...
	// Then an error from ParseFloat and no VideoInfo
	require.Error(t, err)
